...
/// 单资产跌破择时：当前收盘价跌破过去 lookback 个交易日最低收盘价则转入防守资产或空仓，否则继续持有基准资产。
pub fn select_breakdown_timing_single(
    ...
    let breakdown_level = ((i - lookback)..i)
        .map(|idx| bars.get(&dates[idx]).unwrap().close)
        .fold(f64::MAX, f64::min);

    if current_bar.close <= breakdown_level {
...
mod tests {
    ...
    #[test]
    fn holds_benchmark_without_breakdown() {
        ...
        let selected = select_breakdown_timing_single(&maps, &dates, 3, 3, "hs300", None);
        assert_eq!(selected, vec!["hs300".to_string()]);
    }

    #[test]
    fn falls_back_to_defensive_after_breakdown() {
        let dates = sample_dates();
        let mut maps = HashMap::new();
        maps.insert(
            "hs300".to_string(),
        ...
        let selected =
            select_breakdown_timing_single(&maps, &dates, 3, 3, "hs300", Some("dividend"));
