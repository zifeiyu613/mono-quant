...
这三者都比 `momentum_topn` 更成熟，也比 `risk_off_rotation` 更不容易把组合过度集中到单一逻辑上。
...
| 2 | `volatility_adjusted_momentum` | 28.57% | -20.20% | 42 | 否 | 新增后表现最强的主动轮动候选 |
| 3 | `breakout_timing_single` | 27.54% | -16.52% | 23 | 否 | 新增后表现最强的低复杂度运行候选 |
| 4 | `absolute_momentum_breadth` | 21.79% | -18.46% | 45 | 否 | 第四批新增后直接进入一线候选 |
| 5 | `dual_momentum` | 20.79% | -18.92% | 41 | 否 | 当前仍然是核心主动候选 |
| 6 | `buy_hold_single` | 18.27% | -38.47% | 1 | 否 | 单资产长期持有基准 |
| 7 | `ma_timing_single` | 8.06% | -20.68% | 27 | 否 | 低复杂度运行候选 |
| 8 | `breakout_rotation_topn` | 5.93% | -17.49% | 7 | 否 | 低换手、未停机，但收益偏弱 |
| 9 | `reversal_bottomn` | 17.61% | -19.86% | 48 | 是 | 适合做反转对照，不适合先前推 |
| 10 | `relative_strength_pair` | 16.48% | -20.63% | 28 | 是 | 轻量双资产切换，对照价值大于当前运行价值 |
| 11 | `absolute_momentum_single` | 5.71% | -19.93% | 27 | 是 | 可保留，但暂不前推 |
| 12 | `momentum_topn` | -6.17% | -19.24% | 38 | 是 | 当前样本下弱化 |
| 13 | `risk_off_rotation` | -9.78% | -21.78% | 37 | 是 | 当前样本下弱化 |
...
#### 观察 2：新增后的主动轮动里，`volatility_adjusted_momentum` 很值得重点追踪
...
#### 观察 3：`absolute_momentum_breadth` 是第四批里最值得继续追踪的新候选
...
**`volatility_adjusted_momentum` ≈ `dual_momentum` > 其他主动策略**

#### 观察 5：低复杂度运行候选里，`breakout_timing_single` 当前最强
...
#### 观察 6：`breakout_rotation_topn` 适合保留，但更像低换手观察组
...
#### 观察 7：`relative_strength_pair` 有保留价值，但当前更像审核友好型对照
...
#### 观察 8：`reversal_bottomn` 适合保留在研究池，作为反命题验证
...
#### 观察 9：`momentum_topn` 和 `risk_off_rotation` 暂时不适合继续前推
...
- 主动主候选：`volatility_adjusted_momentum`、`absolute_momentum_breadth`、`dual_momentum`
...
2. 把 `volatility_adjusted_momentum` 和 `absolute_momentum_breadth` 放在同一梯队继续对照
...
7. 暂时不把 `reversal_bottomn` / `momentum_topn` / `risk_off_rotation` 继续往运行层前推
8. 后续再补：
-   人工覆写
-   执行回写
-   paper trading
