...
如果已经进入 P1，建议同时补一个最小 `risk` 段：

```json
{
  "risk": {
    "min_aligned_days": 250,
    "max_single_asset_weight": 0.5,
    "max_daily_loss_limit": 0.08,
    "max_drawdown_limit": 0.15,
    "max_rebalance_turnover": 0.8,
...
当前这组风控含义分别是：
- `min_aligned_days`：最小可用样本门槛
- `max_single_asset_weight`：单资产权重上限
- `max_daily_loss_limit`：单日组合亏损停机线
- `max_drawdown_limit`：组合最大回撤停机线
- `max_rebalance_turnover`：单次调仓换手上限
- `stop_cooldown_days`：触发停机后的空仓冷静期，结束后恢复到后续调仓点

其中 `max_single_asset_weight` 还会隐含一个最小资产池要求：
- `0.50` 表示至少要有 `2` 个可交易资产
- `0.40` 表示至少要有 `3` 个可交易资产
- `0.25` 表示至少要有 `4` 个可交易资产

如果当前资产池数量连这个最小要求都达不到，系统会直接报错或在运行时触发停机，而不是静默突破权重上限。
